...
COMMENT ON SCHEMA memory IS 'Схема долговременной памяти агента';
...
DO $$ BEGIN
IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'public.dialog_status') THEN
...
DO $$ BEGIN
IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'public.dialog_close_reason') THEN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'state.shutdown_type') THEN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'state.agent_state_type') THEN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'state.lifecycle_change_reason') THEN
...
-- Создание ENUM для статуса верификации гипотез
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'memory.hypothesis_status') THEN
        CREATE TYPE memory.hypothesis_status AS ENUM (
    ...
    END IF;
END $$;
COMMENT ON TYPE memory.hypothesis_status IS 'Стадии жизненного цикла гипотезы';

-- Создание ENUM дял Источник знания гипотез
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'memory.knowledge_source') THEN
        CREATE TYPE memory.knowledge_source AS ENUM (
            'user',      -- Факт сообщил пользователь
            'agent',     -- Факт предложен ассистентом (совет, рекомендация)
            'external'   -- Внешний контекст (новость, статистика, закон)
        );
    END IF;
END $$;
COMMENT ON TYPE memory.knowledge_source IS 'Источник извлечённого знания';

-- Создание ENUM для источника метрики LLM
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'public.metric_source') THEN
        CREATE TYPE public.metric_source AS ENUM ('internal', 'external');
    END IF;
END $$;

-- Создание ENUM статусов интеграции гипотез
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'memory.graph_merge_status') THEN
        CREATE TYPE memory.graph_merge_status AS ENUM ('none', 'pending_llm', 'integrated', 'needs_review', 'route_locked');
    END IF;
END $$;
COMMENT ON TYPE memory.graph_merge_status IS 'Статус интеграции гипотезы в граф: none-не обработана, pending_llm-ждёт LLM, integrated-в графе, needs_review-ручной разбор, route_locked-в обработке';
...
VALUES 
    ...
    ('form_classification', 'LLM-классификация draft-гипотез по справочнику memory.forms'),
    ('entity_clustering', 'Батчевая кластеризация fact-узлов и создание entity-агрегаторов'),
    ('entity_binding', 'Инкрементальная привязка новых fact-узлов к существующим entity')
...
VALUES
    ...
    ('graph_merge_ctx', 'Загрузка контекста узла и гипотезы для LLM-слияния', '1.3.0'),
    ('graph_merge_parse', 'Парсинг и валидация JSON-ответа LLM при слиянии', '1.3.0'),
    ('form_classification', 'Батчевая классификация гипотез и присвоение form_code', '1.4.0'),
    ('entity_cluster_load',  'Загрузка fact-узлов с needs_entity_binding=TRUE', '1.4.0'),
    ('entity_cluster_llm',   'LLM-именование кластера сущностей', '1.4.0'),
    ('entity_cluster_tx',    'Транзакция создания entity-узла и about-рёбер', '1.4.0'),
    ('entity_binding_load',  'Загрузка новых fact-узлов для привязки', '1.4.0'),
    ('entity_binding_match', 'Поиск ближайших entity-узлов через Qdrant', '1.4.0'),
    ('entity_binding_tx',    'Транзакция создания about-ребра', '1.4.0')
...
-- Базовый системный промпт личности агента для внешних api
INSERT INTO orchestrator.prompts (
    version, name, description, type, destination_id,
    text, params, prompt_effectiveness, status, created_by, agent_version, created_at
) VALUES (
    '1.1.0',
    'agent_core_identity_external',
    'Базовый системный промпт агента для внешних api',
    'external'::public.prompt_type,
    (SELECT id FROM orchestrator.prompt_destinations WHERE name = 'system'),
    E'<Правила>\n' ||
    E'Ты — универсальный ассистент-эксперт по практическим жизненным вопросам.\n' ||
    E'Отвечай кратко, по существу, максимально полезно.\n' ||
    E'Сначала давай прямой ответ, затем, если требуются необходимые пояснения.\n' ||
    ...
    '{}'::jsonb,
    'testing'::public.prompt_status,
    (SELECT id FROM users.actors WHERE type = 'owner' LIMIT 1),
    '1.1.0',
    now()
);


-- Промпт экстракции знаний в долговременную память
INSERT INTO orchestrator.prompts (
    version, name, description, type, destination_id,
    text, params, prompt_effectiveness, status, created_by, agent_version, created_at
) VALUES (
    '1.1.0',
    'memory_hypothesis_extractor',
    ...
    E'Ты — анализатор диалогов для базы знаний. Извлеки из этого разговора ВСЕ значимые факты, планы, рекомендации и договорённости, которые пригодятся для планирования дел, базы навыков, справочных данных, анализа или дальнейшей работы.\n' ||
    E'ЗНАЧИМЫМИ СЧИТАЮТСЯ:\n' ||
    E'- Конкретные факты о пользователе, его предпочтения, ограничения, привычки. Учитывай все уточнения и исправления по ходу диалога (последние версии).\n' ||
    E'- Суть любых советов и планов, которые даёт ассистент. Преобразуй их в утверждения «Рекомендуется…» или «Для достижения … нужно…».\n' ||
    E'- Договорённости о роли ассистента, стиле общения, запретах (например, «работай как цифровая система», «не прогнозируй даты»).\n' ||
    E'- Внешний контекст: новости, законы, экономические данные, полезные знания.\n\n' ||
    E'ЧТО ДЕЛАТЬ С ИМПЕРАТИВНЫМИ СОВЕТАМИ АССИСТЕНТА:\n' ||
    E'Любое предложение вида «Выпиши…», «Раздели…», «Заморозь…» переводи в нейтральное знание.\n' ||
    E'Пример: «Перестаньте брать новые задачи» → «Для наведения порядка ассистент рекомендует временно прекратить брать новые задачи».\n\n' ||
    E'ВАЖНЫЕ ПРАВИЛА:\n' ||
    E'- Каждый самостоятельный шаг, правило или действие — ОТДЕЛЬНАЯ гипотеза. Не объединяй несколько рекомендаций в одну, даже если они в одном сообщении.\n' ||
    E'- Для многошаговых методик — столько гипотез, сколько шагов.\n' ||
    E'- Обязательно проверяй, учтены ли все тактические советы ассистента из диалога. Если ассистент перечислял действия, вытащи каждое.\n\n' ||
    E'НЕ ИЗВЛЕКАТЬ: приветствия, прощания, общие фразы без практической пользы.\n' ||
    E'Если в чанке значимых знаний нет — верни []\n\n' ||
    E'УКАЖИ ИСТОЧНИК ЗНАНИЯ (knowledge_source):\n' ||
    E'- "user" — факт сообщил пользователь.\n' ||
    E'- "agent" — факт предложен ассистентом (совет, план, рекомендация, договорённость о роли).\n' ||
    E'- "external" — внешний контекст, упомянутый любым участником.\n\n' ||
    E'КЛАССИФИЦИРУЙ ПО ДОМЕНАМ (domain_code): {domains}\n' ||
    E'Если ни один домен не подходит, указывай "general".\n' ||
    E'source_message_ids ДОЛЖНЫ ТОЧНО ССЫЛАТЬСЯ НА ИСТОЧНИК.\n\n' ||
    E'ОЦЕНИ УВЕРЕННОСТЬ (confidence) от 0.0 до 1.0:\n' ||
    E'0.85–1.0 — прямо сказано.\n' ||
    E'0.60–0.84 — ясно следует из контекста.\n' ||
    E'<0.3 — не включай.\n\n' ||
    ...
    E']\n\n' ||
    E'ПРИМЕРЫ:\n' ||
    E'1. Разложение плана ассистента на шаги (критически важный):\n' ||
    E'Ассистент: ''В течение недели выпишите все 100 проектов на бумагу или в файл без фильтров. Разделите проекты на три группы: закрыть навсегда, отложить на год, развивать сейчас (не более трех штук). Перестаньте брать новые задачи до конца третьего месяца. Установите жесткий график дня, включающий время для себя и поиск партнера.''\n' ||
    E'→\n' ||
    E'[\n' ||
    E'  {\n' ||
    E'    "hypothesis_text": "Ассистент рекомендует в течение недели выписать все 100 незавершённых проектов без фильтрации",\n' ||
    E'    "domain_code": "planning",\n' ||
    E'    "confidence": 0.95,\n' ||
    E'    "source_message_ids": ["uuid_сообщения_ассистента"],\n' ||
    E'    "knowledge_source": "agent"\n' ||
    E'  },\n' ||
    E'  {\n' ||
    E'    "hypothesis_text": "Ассистент рекомендует разделить выписанные проекты на три категории: закрыть навсегда, отложить на год, развивать сейчас (не более трёх)",\n' ||
    E'    "domain_code": "planning",\n' ||
    E'    "confidence": 0.95,\n' ||
    E'    "source_message_ids": ["uuid_сообщения_ассистента"],\n' ||
    E'    "knowledge_source": "agent"\n' ||
    E'  },\n' ||
    E'  {\n' ||
    E'    "hypothesis_text": "Ассистент рекомендует прекратить брать новые задачи до конца третьего месяца",\n' ||
    E'    "domain_code": "planning",\n' ||
    E'    "confidence": 0.7,\n' ||
    E'    "source_message_ids": ["uuid_сообщения_ассистента"],\n' ||
    E'    "knowledge_source": "agent"\n' ||
    E'  },\n' ||
    E'  {\n' ||
    E'    "hypothesis_text": "Ассистент рекомендует установить жёсткий график дня, включив время для себя и поиска партнёра",\n' ||
    E'    "domain_code": "planning",\n' ||
    E'    "confidence": 0.9,\n' ||
    E'    "source_message_ids": ["uuid_сообщения_ассистента"],\n' ||
    E'    "knowledge_source": "agent"\n' ||
    E'  }\n' ||
    E']\n\n' ||
    E'2. Договорённость о роли:\n' ||
    E'Пользователь: ''Я же вроде бы ясно сказал, что ты и есть цифровая система знаний…''\n' ||
    E'Ассистент: ''Извини, я слишком рано переключилась на тактику. Ты прав…''\n' ||
    E'→\n' ||
    E'[\n' ||
    E'  {\n' ||
    E'    "hypothesis_text": "Ассистент должен работать в режиме цифровой системы хранения и структурирования, не давая тактических советов без согласования",\n' ||
    E'    "domain_code": "agent_profile",\n' ||
    E'    "confidence": 0.6,\n' ||
    E'    "source_message_ids": ["uuid...", "uuid..."],\n' ||
    E'    "knowledge_source": "agent"\n' ||
    E'  }\n' ||
    E']\n\n' ||
    E'3. Цели пользователя с учётом уточнений:\n' ||
    E'Пользователь: ''Хочу дом в деревне.'' → Позже: ''Участок 15 соток уже есть.''\n' ||
    E'→\n' ||
    E'[\n' ||
    E'  {\n' ||
    E'    "hypothesis_text": "Пользователь имеет участок 15 соток в 120 км от города и хочет построить на нём дом",\n' ||
    E'    "domain_code": "user_profile",\n' ||
    E'    "confidence": 1.0,\n' ||
    E'    "source_message_ids": ["uuid_первого_сообщения", "uuid_уточнения"],\n' ||
    E'    "knowledge_source": "user"\n' ||
    E'  }\n' ||
    E']\n\n' ||
    E'4. Запрет жёстких сроков:\n' ||
    E'Пользователь: ''Прогнозировать дату завершения дел — это утопия. У меня нет жестких сроков для целей.''\n' ||
    E'→\n' ||
    E'[\n' ||
    E'  {\n' ||
    E'    "hypothesis_text": "При планировании не следует устанавливать жёсткие сроки для целей, так как пользователь считает это нереалистичным",\n' ||
    E'    "domain_code": "user_profile",\n' ||
    E'    "confidence": 0.9,\n' ||
    E'    "source_message_ids": ["uuid_сообщения"],\n' ||
    E'    "knowledge_source": "user"\n' ||
    E'  }\n' ||
    E']',
    '{
...
COMMENT ON COLUMN metrics.llm_artifacts.metric_source IS 'Источник метрики: внутренняя / внешняя модель.';
...
('inactivity_sleep_minutes', 20.0,  'Число минут без сообщений пользователей, после которых агент переходит из active в sleep.'),
('dialogue_inactivity_timeout_minutes', 15.0, 'Таймаут неактивности диалога в минутах. Если last_activity_at старше порога, диалог закрывается и создаётся новый.'),
('graph_pipeline_interval_minutes', 30.0, 'Минимальный интервал между запусками graph_route_and_create (минуты). Защита от запуска после каждой верифицированной гипотезы.'),
('graph_summarize_interval_minutes', 30.0, 'Минимальный интервал между запусками graph_summarize (минуты). Защита от бесконечного цикла суммаризаций.'),
('verification_defer_minutes', 30.0, 'Минуты, на которые откладывается верификация при выборе "отложить"'),
('entity_clustering_interval_minutes', 20.0, 'Интервал запуска батчевой кластеризации entity-узлов (минуты). 0 = отключено.')
...
-- 4.19 Справочник форм сущностей (forms) — определяет природу узла
CREATE TABLE IF NOT EXISTS memory.forms (
    ...
    agent_version TEXT NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT unique_forms_name UNIQUE (name)
);

COMMENT ON TABLE memory.forms IS 'Справочник форм сущностей в графе знаний. Определяет природу узла (что это), независимо от домена.';
COMMENT ON COLUMN memory.forms.code IS 'Системный код формы: fact, goal, task, project, entity, skill, event.';
COMMENT ON COLUMN memory.forms.name IS 'Человекочитаемое название формы.';
COMMENT ON COLUMN memory.forms.description IS 'Описание: какие сущности относятся к этой форме.';
COMMENT ON COLUMN memory.forms.is_active IS 'Активна ли форма. Неактивные не используются для новых узлов.';
COMMENT ON COLUMN memory.forms.agent_version IS 'Версия агента, при которой форма была добавлена/изменена.';

CREATE INDEX IF NOT EXISTS idx_forms_name ON memory.forms (name);
CREATE INDEX IF NOT EXISTS idx_forms_is_description ON memory.forms (description);
CREATE INDEX IF NOT EXISTS idx_forms_is_active ON memory.forms (is_active);
CREATE INDEX IF NOT EXISTS idx_forms_name_is_active ON memory.forms (name, is_active);

INSERT INTO memory.forms (code, name, description, agent_version) VALUES
('fact',    'Факт',      'Констатация: "У меня есть BMW X5 2022 года". Утверждение о текущем состоянии или свойстве.', '1.4.0'),
('goal',    'Цель',      'Намерение без дедлайна: "Стать финансово независимым". Долгосрочное стратегическое желание.', '1.4.0'),
('task',    'Задача',    'Конкретное действие с дедлайном: "Оплатить страховку до пятницы". Тактический шаг.', '1.4.0'),
('project', 'Проект',    'Сквозная инициатива с критерием завершения: "Переезд в Грузию". Группирует цели, задачи и ресурсы.', '1.4.0'),
('entity',  'Сущность',  'Сущность реального мира: человек, машина, компания, книга. Именованный объект с устойчивой идентичностью.', '1.4.0'),
('skill',   'Навык',     'Алгоритм "как делать": "Как вести переговоры о зарплате". Инструкция или накопленный опыт.', '1.4.0'),
('event',   'Событие',   'Событие во времени: "Увольнение 12.03.2026", "Встреча с Иваном". Факт с датой или интервалом.', '1.4.0'),
('entity', 'Сущность', 'Именованный объект реального мира (человек, машина, книга, организация). Агрегирует fact-узлы через связь about.', '1.4.0')
ON CONFLICT (code) DO NOTHING;


-- 4.20 Справочник доменов знаний (тематические группы)
CREATE TABLE IF NOT EXISTS memory.knowledge_domains (
    ...
    agent_version TEXT NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT unique_knowledge_domains_name UNIQUE (name)
);
COMMENT ON TABLE memory.knowledge_domains IS 'Справочник тематических сфер жизни для классификации гипотез. Расширяется вручную.';
COMMENT ON COLUMN memory.knowledge_domains.id IS 'Уникальный идентификатор записи (UUID)';
COMMENT ON COLUMN memory.knowledge_domains.code IS 'Системный код домена (tasks, budget, survival, ...)';
COMMENT ON COLUMN memory.knowledge_domains.name IS 'Человекочитаемое название тематической группы (например, "Бюджет", "Выживание")';
COMMENT ON COLUMN memory.knowledge_domains.description IS 'Развёрнутое описание домена, его границ и назначения';
COMMENT ON COLUMN memory.knowledge_domains.is_active IS 'Активен ли домен. Неактивные не предлагаются экстрактору.';
COMMENT ON COLUMN memory.knowledge_domains.agent_version IS 'Версия агента, с которой связан данный домен (используется для фильтрации актуальных справочников)';
COMMENT ON COLUMN memory.knowledge_domains.created_at IS 'Метка времени создания записи (UTC)';


-- Индекс для быстрой фильтрации только активных доменов (наиболее частый сценарий)
CREATE INDEX IF NOT EXISTS idx_knowledge_domains_code ON memory.knowledge_domains (code);
CREATE INDEX IF NOT EXISTS idx_knowledge_domains_name ON memory.knowledge_domains (name);
CREATE INDEX IF NOT EXISTS idx_knowledge_domains_description ON memory.knowledge_domains (description);
CREATE INDEX IF NOT EXISTS idx_knowledge_domains_is_active ON memory.knowledge_domains (is_active);

INSERT INTO memory.knowledge_domains (code, name, description, agent_version) VALUES
    ('health', 'Медицина и Биохимия', 'Состояние организма. Диагнозы, врачи, анализы, генетика, сон, уровень стресса, аллергии, лекарства, медицинаская помощь.', '1.2.0'),
    ('sport', 'Тело и Нагрузки', 'Физическая активность. Тренировки, графики, нормативы, спортивные инвентарь, единоборства, биомеханика.', '1.2.0'),
    ('food', 'Питание и Рацион', 'Топливо для тела. БЖУ, калорийность, рецепты, диеты, БАДы, сроки годности продуктов, запасы продуктов, питание в походах.', '1.2.0'),
    ('finance', 'Финансы и Денежные ресурсы.', 'Деньги, активы, бюджеты. Доходы, расходы, инвестиции, налоги, криптокошельки. Не путать с физическими объектами (машинами/квартирами).', '1.2.0'),
    ('world_state', 'Внешний Мир и Контекст', 'Новости, политика, макроэкономика, законы, тренды, погода, книги, фильмы, музыка, общие знания не входящие в другие категории.', '1.2.0'),
    ('society', 'Общество, Люди и межличностные связи', 'Социальный граф. Контакты, психотипы, уровень доверия, история взаимодействий, семья, коллеги, друзья, наблюдения за людьми.', '1.2.0'),
    ('safety', 'Безопасность', 'Физическая, цифровая и правовая безопасность человека, потребность в жилище, угрозы, инциденты, противодействие, автономное выживание в современном мире.', '1.2.0'),
    ('hobby', 'Хобби, творчество, отдых', 'Творчество, игры, путешествия, рукоделие, коллекционирование, культурные мероприятия, развитие талантов.', '1.2.0'),
    ('user_profile', 'Психография и Личность пользователя', 'Внутренний мир пользователя. Ценности, характер, триггеры, страхи, предпочтения, биография, привычки, любая личная информация.', '1.2.0'),
    ('agent_profile', 'Мета-память Агента о себе', 'Самосознание системы агента о себе. Системные лимиты, инструкции по стилю общения, имя, ошибки прошлых диалогов, настройки поведения.', '1.2.0'),
    ('skills', 'Навыки и Умения, Оыт', 'Алгоритмы действий. Инструкции "Как делать", стандартные операционные процедуры, накопленный опыт в решении задач по различным сферам.', '1.2.0')
ON CONFLICT (code) DO NOTHING;


-- 4.21 Строгий справочник тем (иерархия подтем внутри доменов)
...
COMMENT ON COLUMN memory.topics.description IS 'Подробное описание темы, включая её границы и примеры использования.';
COMMENT ON COLUMN memory.topics.created_at IS 'Метка времени создания записи в справочнике.';
COMMENT ON COLUMN memory.topics.updated_at IS 'Метка времени последнего обновления записи.';
COMMENT ON CONSTRAINT unique_topic_name ON memory.topics IS 'Гарантирует, что название темы не повторяется в справочнике.';

-- Дополнительный индекс для частых запросов по дате обновления
CREATE INDEX IF NOT EXISTS idx_topics_name ON memory.topics (name);
CREATE INDEX IF NOT EXISTS idx_topics_description ON memory.topics (description);
...
INSERT INTO memory.topics (name, description) VALUES
    ('news', 'Новости, СМИ, слухи.'),
    ('articles', 'Научные и аналитические статьи, тематические публикации.'),
    ('place', 'Местоположение. Географические данные: текущее место жительства, адреса, регионы.'),
    ('transport', 'Личный транспорт: автомобили, мотоциклы, велосипеды и прочий.'),
    ('property', 'Физические объекты владения. Имущество: вещи, запасы, недвижимое и движимое имущество, техника, гаджеты, запасы, их характеристики.'),
    ('russia', 'Все о ситуации в стране Россия: политика, тренды, проблематика, жизнь в России.'),
    ('bad_habits', 'Вредные привычки. Любые вредные и деструктивные с точки зрения логики привычки.'),
    ('good_habits', 'Полезные привычки. Любые полезные и практичные с точки зрения логики привычки.'),
    ('books', 'Книги и все что связано с литературой. Авторы книг, цитаты из книг, названия книг, обзоры книг.'),
    ('phobias', 'Любые личные комплексы, страхи, фобии.'),
    ('budgets', 'Финансовые бюджеты и бюджетирование. Ведение бюджета, бюджетное планирование, доходы, расходы.'),
    ('credits', 'Финансовые кредиты. Займы, процентные ставки по кредитам, условия кредитов, графики погашения кредитов, реструкторизация кредитов.'),
    ('goals', 'Цели и Стратегия. Стратегические "Зачем" и "Кем стать". Долгосрочные намерения, миссия, видение будущего. НЕ содержат конкретных шагов. (Пример: "Стать финансово независимым", "Выучить английский").'),
    ('tasks', 'Задачи и Планы. Тактические "Что сделать" и "Когда". Конкретные шаги, чек-листы, дедлайны, статусы выполнения, рутина. (Пример: "Оплатить курсы до пятницы", "Позвонить в банк").'), 
    ('projects', 'Жизненные Проекты. Группируют на макро-уровне цели, задачи, финансы и людей для достижения одного результата. (Пример: "Переезд в другую страну", "Запуск стартапа", “Постройка дома””).'),
    ('work', 'Наемная работа, личный бизнес, карьера, службные обязанности.'),
    ('recipes', 'Кулинарные рецепты. Рецепты приготовления пищи, консервов.'),
    ('вentistry', 'Стоматология, лечение и протезирование зубов.'),
    ('daily_routine', 'Распорядок дня, режи сна и отдыха, ценность сна, оптимальнео время суток для выполнения различных задач.'),
    ('physical training', 'Общая физическая подготовка человека: утренняя зарядка, закаливание организма, аэробные нагрузки, растяжка тела, бег, плавание, силовая подготовка.'),
    ('personal', 'Личные данные субъекта: пол, возраст, внешность, вес, рост и тому подобное.')

ON CONFLICT (name) DO NOTHING;


-- 4.22 Таблица гипотез (извлечённых знаний)
...
COMMENT ON COLUMN memory.hypotheses.form_code IS 'Форма сущности (из справочника memory.forms). Заполняется отдельным пайплайном после экстракции, до верификации.';
...
CREATE INDEX idx_hypotheses_form ON memory.hypotheses (form_code) WHERE form_code IS NOT NULL;
...
CREATE INDEX idx_hypotheses_graph_merge_status ON memory.hypotheses (graph_merge_status);
...
CREATE TRIGGER trg_hypotheses_update_updated_at
    BEFORE UPDATE ON memory.hypotheses
    FOR EACH ROW
    EXECUTE FUNCTION common.update_updated_at_column();

-- 4.23 Журнал обработанных СООБЩЕНИЙ (не диалогов!)
CREATE TABLE IF NOT EXISTS memory.message_analyses (
    id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    message_id UUID NOT NULL UNIQUE REFERENCES dialogs.row_messages(id) ON DELETE CASCADE,
    ...
    -- Трассируемость
    orchestrator_step_id UUID REFERENCES orchestrator.orchestrator_steps(id) ON DELETE SET NULL,
    llm_metric_id UUID REFERENCES metrics.llm_internal(id) ON DELETE SET NULL,
    prompt_id UUID NOT NULL REFERENCES orchestrator.prompts(id) ON DELETE RESTRICT,
    
    -- Метаданные
    agent_version TEXT NOT NULL,
    processed_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
COMMENT ON TABLE memory.message_analyses IS 'Журнал: какие сообщения уже проанализированы экстрактором. Prevents double processing.';
COMMENT ON COLUMN memory.message_analyses.message_id IS 'Ссылка на конкретное сообщение. UNIQUE — одно сообщение = одна запись.';
COMMENT ON COLUMN memory.message_analyses.prompt_id IS 'Промпт, которым анализировалось сообщение';
COMMENT ON COLUMN memory.message_analyses.orchestrator_step_id IS 'Шаг оркестратора, выполнивший анализ';
COMMENT ON COLUMN memory.message_analyses.llm_metric_id IS 'Метрика LLM-запроса';

CREATE INDEX idx_message_analyses_message ON memory.message_analyses (message_id);
CREATE INDEX idx_message_analyses_step ON memory.message_analyses (orchestrator_step_id);
CREATE INDEX idx_message_analyses_prompt ON memory.message_analyses (prompt_id);
CREATE INDEX idx_message_analyses_processed_at ON memory.message_analyses (processed_at DESC);


-- 4.24 Таблица метрик ВНЕШНИХ LLM (аналог llm_internal)
CREATE TABLE IF NOT EXISTS metrics.llm_external (
    id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    orchestrator_step_id UUID REFERENCES orchestrator.orchestrator_steps(id) ON DELETE RESTRICT,
    orchestrator_step_name TEXT,
    prompt_id UUID REFERENCES orchestrator.prompts(id) ON DELETE RESTRICT,
...
COMMENT ON TABLE metrics.llm_external IS 'Метрики запросов к внешним LLM API (DashScope, OpenAI и др.)';
COMMENT ON COLUMN metrics.llm_external.id IS 'Уникальный идентификатор записи';
COMMENT ON COLUMN metrics.llm_external.orchestrator_step_id IS 'Идентификатор шага оркестратора, если вызов связан с конкретным шагом';
COMMENT ON COLUMN metrics.llm_external.orchestrator_step_name IS 'Имя шага оркестратора (денормализованное поле для удобства)';
COMMENT ON COLUMN metrics.llm_external.prompt_id IS 'Идентификатор используемого промпта';
COMMENT ON COLUMN metrics.llm_external.provider IS 'Провайдер LLM (например, OpenAI, Anthropic, Cohere)';
COMMENT ON COLUMN metrics.llm_external.model IS 'Название модели (например, gpt-4, claude-3-opus)';
COMMENT ON COLUMN metrics.llm_external.param IS 'Параметры генерации (температура, топ-p, frequency_penalty и т.д.) в формате JSON';
COMMENT ON COLUMN metrics.llm_external.prompt_tokens IS 'Количество токенов, переданных в промпте';
COMMENT ON COLUMN metrics.llm_external.completion_tokens IS 'Количество токенов в ответе модели';
COMMENT ON COLUMN metrics.llm_external.total_tokens IS 'Общее количество токенов (prompt + completion)';
COMMENT ON COLUMN metrics.llm_external.host_nctx IS 'Размер контекстного окна хоста (в токенах), если известно';
COMMENT ON COLUMN metrics.llm_external.prompt_ms IS 'Время обработки промпта на стороне провайдера (в миллисекундах)';
COMMENT ON COLUMN metrics.llm_external.prompt_per_token_ms IS 'Среднее время на один токен промпта (в миллисекундах)';
COMMENT ON COLUMN metrics.llm_external.prompt_per_second IS 'Скорость обработки токенов промпта (токенов в секунду)';
COMMENT ON COLUMN metrics.llm_external.predicted_per_second IS 'Прогнозируемая скорость генерации (токенов в секунду), может рассчитываться провайдером';
COMMENT ON COLUMN metrics.llm_external.resp_time IS 'Общее время получения ответа (включая сеть и обработку)';
COMMENT ON COLUMN metrics.llm_external.net_latency IS 'Чистая сетевая задержка (без времени обработки модели)';
COMMENT ON COLUMN metrics.llm_external.full_time IS 'Полное время выполнения запроса (от отправки до получения полного ответа)';
COMMENT ON COLUMN metrics.llm_external.error_status IS 'Флаг наличия ошибки (true – ошибка, false – успех)';
COMMENT ON COLUMN metrics.llm_external.error_message IS 'Текст ошибки, если произошла (иначе NULL)';
COMMENT ON COLUMN metrics.llm_external.error_time IS 'Время возникновения ошибки (если была)';
COMMENT ON COLUMN metrics.llm_external.agent_version IS 'Версия агента, выполнявшего вызов';
COMMENT ON COLUMN metrics.llm_external.timestamp IS 'Время создания записи (обычно время завершения запроса)';

CREATE INDEX IF NOT EXISTS idx_llm_external_step ON metrics.llm_external (orchestrator_step_id);
CREATE INDEX IF NOT EXISTS idx_llm_external_prompt ON metrics.llm_external (prompt_id);
CREATE INDEX IF NOT EXISTS idx_llm_external_provider ON metrics.llm_external (provider);
CREATE INDEX IF NOT EXISTS idx_llm_external_model ON metrics.llm_external (model);
CREATE INDEX IF NOT EXISTS idx_llm_external_timestamp ON metrics.llm_external (timestamp);

-- Триггер auto-fill step_name (используем уже существующую функцию)
DROP TRIGGER IF EXISTS trg_external_llm_populate_step_name ON metrics.llm_external;
CREATE TRIGGER trg_external_llm_populate_step_name
BEFORE INSERT ON metrics.llm_external
FOR EACH ROW
EXECUTE FUNCTION metrics.populate_llm_step_name();

-- ============================================================
-- orchestrator.orchestrator_steps: ТОЛЬКО ДОБАВЛЯЕМ поля рядом
-- llm_metric_id НЕ ТРОГАЕМ (остался FK на llm_internal)
-- ============================================================
ALTER TABLE orchestrator.orchestrator_steps 
    ADD COLUMN IF NOT EXISTS llm_metric_external_id UUID REFERENCES metrics.llm_external(id) ON DELETE SET NULL,
    ADD COLUMN IF NOT EXISTS metric_source metric_source DEFAULT NULL;
   

CREATE INDEX IF NOT EXISTS idx_steps_llm_external ON orchestrator.orchestrator_steps (llm_metric_external_id);

COMMENT ON COLUMN orchestrator.orchestrator_steps.llm_metric_external_id IS 'FK на metrics.llm_external (заполнено когда metric_source=external)';
COMMENT ON COLUMN orchestrator.orchestrator_steps.metric_source IS 'Источник метрики: internal (llm_metric_id заполнен) или external (llm_metric_external_id заполнен)';

-- CHECK: только одно из двух полей заполнено
ALTER TABLE orchestrator.orchestrator_steps
    ADD CONSTRAINT chk_steps_metric_source CHECK (
        (metric_source IS NULL) OR
        (metric_source = 'internal' AND llm_metric_id IS NOT NULL AND llm_metric_external_id IS NULL) OR
        (metric_source = 'external' AND llm_metric_external_id IS NOT NULL AND llm_metric_id IS NULL)
    );

-- ============================================================
-- metrics.llm_artifacts: ТОЛЬКО ДОБАВЛЯЕМ поля рядом
-- llm_metric_id НЕ ТРОГАЕМ (остался FK на llm_internal)
-- ============================================================
ALTER TABLE metrics.llm_artifacts
    ADD COLUMN IF NOT EXISTS llm_metric_external_id UUID REFERENCES metrics.llm_external(id) ON DELETE CASCADE,
    ADD COLUMN IF NOT EXISTS metric_source metric_source DEFAULT NULL;

-- Вынужденный ALTER: снимаем NOT NULL с llm_metric_id,
-- чтобы external-записи могли иметь llm_metric_id = NULL.
-- Старые записи не затрагиваются — у них уже есть значение.
ALTER TABLE metrics.llm_artifacts ALTER COLUMN llm_metric_id DROP NOT NULL;

CREATE INDEX IF NOT EXISTS idx_artifacts_external ON metrics.llm_artifacts (llm_metric_external_id);

COMMENT ON COLUMN metrics.llm_artifacts.llm_metric_external_id IS 'FK на external-метрику (заполнено когда metric_source=external)';
COMMENT ON COLUMN metrics.llm_artifacts.metric_source IS 'Источник: internal (llm_metric_id заполнен) или external (llm_metric_external_id заполнен)';

-- CHECK: в зависимости от metric_source заполнено нужное поле
ALTER TABLE metrics.llm_artifacts
    ADD CONSTRAINT chk_artifacts_metric_source CHECK (
        (metric_source = 'internal' AND llm_metric_id IS NOT NULL AND llm_metric_external_id IS NULL) OR
        (metric_source = 'external' AND llm_metric_external_id IS NOT NULL AND llm_metric_id IS NULL)
    );